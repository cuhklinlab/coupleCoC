function [Cx] = updateRowClustering_old(p, tilde_p, Cx, cluster_p, cluster_q, beta, indicator)
...
for rc = 1:size(tilde_pZtx, 1)
  for r = 1:size(pZx, 1)
    temp(rc, r) = KLDiv(pZx(r,:), tilde_pZtx(rc,:)) + beta * JSDiv_pc(cluster_p, cluster_q, rc,indicator) / (sum(p(r,:),2) * size(tilde_pZtx, 1));
  end
end
